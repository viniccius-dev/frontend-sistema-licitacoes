...
export function EditBidding() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await api.put(`/bids/${params.id}`, biddingData);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const attachmentsResponse = await api.get(`/bids/attachments/${params.id}`);
                setAttachments(attachmentsResponse.data);
    
                const bidResponse = await api.get(`/bids/${params.id}`);
                const [fullDate, fullTime] = bidResponse.data.realized_at.split(' ');
                const [year, month, day] = fullDate.split('-');
                const formattedDate = `${day}/${month}/${year}`;
    
                setBid(bidResponse.data);
                setSelectedModality(listModalities.find(modality => modality.name === bidResponse.data.bidding_modality));
                setNumModalityProcess(bidResponse.data.modality_process_number);
                setSelectedStatus(listStatus.find(status => status.name === bidResponse.data.status));
                setNumBidProcess(bidResponse.data.bidding_process_number);
                setDate(formattedDate);
                setHour(fullTime);
                setObject(bidResponse.data.object);
                setObservations(bidResponse.data.observations);
            } catch (error) {
                if (error.response) {
                    alert(error.response.data.message);
                } else {
