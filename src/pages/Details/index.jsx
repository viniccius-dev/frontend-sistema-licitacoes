...
export function Details() {
    const attachments = [
        { title: 'Documento de Licitação', extension: '.pdf' },
        { title: 'Planilha de Custos', extension: '.xls' },
        { title: 'Contrato', extension: '.docx' }
    ];

    return (
        <Fixed title="Licitação" route="/">
            <Container>
                <header>
                ...
                <Section title="Anexos">
                    {attachments.map((file, index) => (
                        <File key={index} title={file.title} extension={file.extension} />
                    ))}
                </Section>
            </Container>
        </Fixed>
